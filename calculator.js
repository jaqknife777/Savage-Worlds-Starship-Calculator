...
function updateWeaponFormVisibility() {
	...
	if (showLevel) {
	  ...
	  // clamp current input if it exceeds the cap
	...
	// Missiles/Torpedoes use Launchers + Ammo
...
equippedWeapons.forEach(ew => {
  ...
  const isAmmoType = (w.type === 'missile' || w.type === 'torpedo');

  const mods     = weaponModsFor(w, ew.level, ew.qty, ew.ammo, ew.qty);
  const baseCost = weaponCostFor(w, ew.level, data.size, ew.qty, ew.ammo, ew.qty);
  const entryCost = isAmmoType ? baseCost : baseCost * ew.qty; // <-- multiply mounts for fixed/level
...
function weaponModsFor(w, level, qty, ammo, launchers){
  if (w.type === 'level') {
    const lvl = Math.max(1, parseInt(level||1,10));
    return Math.ceil(lvl/2) * Math.max(1, parseInt(qty||1,10)); // per mount
  }
  if (w.type === 'missile' || w.type === 'torpedo' || w.type === 'bombs') {
    const bays = Math.max(1, parseInt(launchers||1,10)); // launchers or bomb bays
    ...
    const bayMods = (w.launcherModPer || 1) * bays; // default 1 Mod per bay/launcher
    return ammoMods + bayMods;
  }
  // Fixed mounts: Mods × mounts
  return (w.mods || 0) * Math.max(1, parseInt(qty||1,10));
}
...
function addWeapon(){
  ...
  if (!w) return;
	...
	if (w.type === 'missile' || w.type === 'torpedo' || w.type === 'bombs') {
	  const launchers = Math.max(1, parseInt(weaponUI.launchers.value || '1', 10)); // ≥1 bay
	  const ammo = Math.max(0, parseInt(weaponUI.ammo.value || '0', 10));
	  equippedWeapons.push({ key, qty: launchers, ammo, level: 1 });
	  ...
	  equippedWeapons.push({ key, qty, level, ammo: 0 });
  } else {
    // fixed
    const qty = Math.max(1, parseInt(weaponUI.qty.value || '1', 10));
    equippedWeapons.push({ key, qty, level: 1, ammo: 0 });
  }
...
function renderWeapons(){
  ...
  equippedWeapons.forEach((ew, i) => {
    ...
    const cols = [
      ...
      String(mods),
      '$' + Number(entryCost).toLocaleString(),   // <-- show multiplied cost
      w.notes,
      String(ew.qty),                             // qty = launchers for missiles; mounts for others
...
if (weaponUI.listResult){
  ...
  equippedWeapons.forEach(ew => {
	...
	if (w.notes && w.notes.trim()) {
	  line += ` <span class="weapon-note">— ${w.notes}</span>`;
	}
	const li = document.createElement('li');
	li.innerHTML = line;         // keeps it safe/plain text
