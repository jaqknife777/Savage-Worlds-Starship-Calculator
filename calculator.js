...
document.addEventListener("DOMContentLoaded", () => {
	...
	const WEAPONS = [
		// Auto-Cannons
		{
			...
			mods: 1,
			cost: 50000,
			notes: 'AP 4, HW, Reaction Fire. Up to 20mm. Reload $200.'
		},
		{
			key: 'ac_medium',
			...
			mods: 1,
			cost: 100000,
			notes: 'AP 6, HW. 21–30mm. Reload $400.'
		},
		{
			key: 'ac_heavy',
			...
			mods: 2,
			cost: 50000,
			notes: 'AP 8, HW. 31–50mm. Reload $1000.'
		},
		...
		// Flamethrower (Heavy)
		{
			...
			mods: 2,
			cost: 1000,
			notes: 'HW; Cone or MBT up to 18”. Targets least Armored area. Fuel pod $60.'
		},
		...
		// Lasers (Vehicular)
		{
			...
			mods: 1,
			cost: 100000,
			notes: 'AP 5, HW, Reaction Fire. Power core $200.'
		},
		{
			key: 'laser_med',
			name: 'Laser (Medium)',
			...
			mods: 2,
			cost: 500000,
			notes: 'AP 10, HW. Power core $1000.'
		},
		{
			key: 'laser_heavy',
			name: 'Laser (Heavy)',
			...
			mods: 3,
			cost: 1000000,
			notes: 'AP 15, HW. Power core $2000.'
		},
		{
			key: 'laser_super',
		...
		},
		{
			...
			mods: 7,
			cost: 4000000,
			notes: 'AP 40, HW. Size 14+ only. Power core $10K.'
		},
		{
			key: 'laser_mega',
			name: 'Laser (Mega)',
			...
			mods: 10,
			cost: 10000000,
			notes: 'AP 50, HW. Size 16+ only. Power core $25K.'
		},
...
function canFixedWeapon(w) {
  ...
  // families: ac_, cannon_, laser_, io_, and Mass Driver (level)
  if (w.key === 'mass_driver') return true;

  const FIXABLE_PREFIXES = ['ac_', 'cannon_', 'laser_', 'io_'];
  return FIXABLE_PREFIXES.some(p => w.key.startsWith(p));
}
  ...
  weaponUI.level.value = String(v);
	...
	function linkFixedBonuses(mode, qty) {
		...
		return {
			...
			dmg,
			shownGroup: q
		}; // show the *actual* qty as the group size
	}
	...
	function clampInputToCapacity(inputEl) {
		...
		let {
			slotsUsed,
			totalCapacity
		} = computeCapacityAndSlots(modCounts);
	...
	function calculateMods() {
		...
		Object.keys(modCounts).forEach(modName => {
			...
			switch (modName) {
				...
				case "Cargo Pod":
				  cargoSpace += 250 * count; // 10 cu ft per pod
				  break;
				...
				case "Crew Space":
					// +20% crew per rank (per slot). Never drop below 1 crew total.
					crew = Math.max(1, Math.ceil(data.crew * Math.pow(1.2, count)));
					break;
...
function addWeapon() {
  ...
  // NEW: modes allowed if qty is 2 or 4 AND (link OR fixed is allowed)
  const allowModes = (qtyVal === 2 || qtyVal === 4) && (canLinkWeapon(w) || canFixedWeapon(w));
  const chosenMode = allowModes ? (weaponUI.linkMode?.value || 'none') : 'none';
  const mode = (chosenMode === 'linked' && canLinkWeapon(w)) || (chosenMode === 'fixed' && canFixedWeapon(w))
    ? chosenMode
    : 'none';
  const group = (mode !== 'none') ? qtyVal : 0;
  ...
  } else if (w.type === 'level') { // Mass Driver
    const maxLvl = getMaxMassDriverLevel();
  ...
  calculateMods();
	...
	if (weaponUI.addBtn) {
		weaponUI.addBtn.addEventListener('click', addWeapon);
	}
	...
	function captureState() {
		...
		const weapons = equippedWeapons.map(w => ({
			...w
		}));
	...
	function applyState(state) {
		...
		// weapons
		equippedWeapons = Array.isArray(state.weapons) ? state.weapons.map(w => ({
			...w
		})) : [];
	...
	function downloadJson() {
		...
		a.href = url;
		a.download = (shipNameInput.value || 'starship') + '.json';
